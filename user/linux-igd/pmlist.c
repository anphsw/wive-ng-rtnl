...
int pmlist_DeletePortMapping(int enabled, char *protocol, char *externalPort, char *internalClient, char *internalPort)
{
	...
	{
	  memset(command, 0, COMMAND_LEN);
	  snprintf(command, COMMAND_LEN, "-D %s -i %s -p %s --dport %s -j DNAT --to %s:%s", g_vars.preroutingChainName, g_vars.extInterfaceName, protocol, externalPort, internalClient, internalPort);
	...
	{
	  memset(command, 0, COMMAND_LEN);
	  snprintf(command, COMMAND_LEN, "-D %s -p %s -d %s --dport %s -j ACCEPT", g_vars.forwardChainName, protocol, internalClient, internalPort);
