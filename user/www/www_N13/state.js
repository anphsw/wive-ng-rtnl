...
var wan0_ipaddr = "<% nvram_get_x("", "wan0_ipaddr"); %>";
...
function check_changed_status(flag){
	...
	if(flag == "initial"){
		...
		if((old_ifWANConnect == 0) && (usb_path1 != "HSDPA")) // WAN port is not plugged.
			parent.showDrSurf("1");
...
function show_banner(L3){// L3 = The third Level of Menu
	...
	//banner_code +='<option value="TW" selected><#LANG_TW#></option>';
...
tabtitle[3] = new Array("", "<#menu5_4_2#>", "<#menu5_4_3#>", "<#menu5_4_4#>");
...
tablink[3] = new Array("", "Advanced_AiDisk_ftp.asp", "Advanced_AiDisk_others.asp", "Advanced_HSDPA_others.asp");
...
function show_menu(L1, L2, L3){
	...
	if(sw_mode == '2' || sw_mode == '3'){
		...
		if(sw_mode == "2"){
			tablink[0].splice(1,5);
			tabtitle[0].splice(1,5);
			menuL2_link[1] = tablink[0][1];
		}else if(sw_mode == "3"){
			tabtitle[0].splice(3,1);//WMode
			tablink[0].splice(3,1);//WMode
		}
