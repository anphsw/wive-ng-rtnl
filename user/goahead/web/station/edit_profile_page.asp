...
function getChannel()
{
	...
	//alert(channel+" bg="+bg_channel+" a="+a_channel);
...
function networkTypeChange(auth, encryp)
{
	...
	else 
	{
		...
		/*if (auth == 0) //open
			...
			document.profile_page.security_adhoc_mode.options.selectedIndex = 2 ;*/
	...
	else
	{
		...
		if (org_profilename != profilename)
		{
			...
			for (end=tmp.indexOf(";"), i=0; i<6 , end>=0; i++, end=tmp.indexOf(";"))
			{
				...
				//alert("tmp="+tmp +" subprofilename= "+ subprofilename +" proiflename= "+profilename+ " start="+start+ " end="+end + " subprofilename.length="+ subprofilename.length + " profilename.length="+profilename.length);
