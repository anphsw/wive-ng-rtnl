...
void btn_setup_save_setting(PKT_SET_INFO_GW_QUICK *pkt)
{
	...
	if (pkt->QuickFlag&QFCAP_WIRELESS)
	{
	   if (!(pkt->QuickFlag&QFCAP_GET))
	   {
		...
		unsigned char *ws;
		ws = (unsigned char *)(&pkt->WSetting);	

		if (ws[32]==AUTHENTICATION_OPEN)
		{
			nvram_set("wl_auth_mode", "open");
			if (ws[33]==ENCRYPTION_WEP64||
			    ws[33]==ENCRYPTION_WEP128)
			{
				sprintf(tmpbuf, "%x", ws[33]);
				nvram_set("wl_wep_x", tmpbuf);
				nvram_set("wl_key_type", "0");
				if(ws[98]>DEFAULT_KEY_4||
				   ws[98]<DEFAULT_KEY_1) 
				   ws[98]=DEFAULT_KEY_1;
				sprintf(tmpbuf,"%x", ws[98]);
				nvram_set("wl_key", tmpbuf);

				ws[98]=0x0;
				nvram_set("wl_key1", &ws[34]);
				nvram_set("wl_key2", &ws[34]);
				nvram_set("wl_key3", &ws[34]);
				nvram_set("wl_key4", &ws[34]);
			}
			else 
			{
		...
		else if(ws[32]==AUTHENTICATION_SHARED)
		{
		...
		else if(ws[32]==AUTHENTICATION_WPA_PSK)
		{
			nvram_set("wl_auth_mode", "psk");
			nvram_set("wl_wpa_mode", "0");
			if (ws[33]==ENCRYPTION_TKIP)
				nvram_set("wl_crypto", "tkip");
			else if (ws[33]==ENCRYPTION_AES)
...
//		else if(pkt->WSetting.Auth==AUTHENTICATION_WPA_PSK2)
		else if(ws[32]==AUTHENTICATION_WPA_PSK2)
		{
			nvram_set("wl_auth_mode", "psk");
			nvram_set("wl_wpa_mode", "0");
...
//			else if (pkt->WSetting.Encrypt==ENCRYPTION_AES)
	...
	if (pkt->QuickFlag&QFCAP_ISP)
	{
		...
		printf("chk pkt isp setting of dnsserver\n");	// tmp test
		if (pkt->ISPSetting.DNSServer1==0 && pkt->ISPSetting.DNSServer2==0)
		{
			printf("set wandnsenable true\n");	// tmp test
			nvram_set("wan_dnsenable_x", "1");
		}
		else
		{
			printf("set wandnsenable false\n");	// tmp test
	...
	//nvram_commit();
fail:
	printf("btn_setup_save_setting\n");	// ham 1205
/*
	{
		int i;
		unsigned char *dd;

		dd = (unsigned char *)(&pkt->WSetting);
		fprintf(stderr, "data: ");
		for(i=0;i<sizeof(GW_QUICK_WIRELESS);i++)
		{
			if ((i%16)==0) fprintf(stderr, "\n");
			fprintf(stderr, "%02x ", dd[i]);
		}
		fprintf(stderr, "\n");
	}
*/
...
int OTSStart(int flag)
{
	...
	else
	{
		...
		eval("iwpriv", "ra0", "set", "AuthMode=OPEN");
		eval("iwpriv", "ra0", "set", "EncrypType=NONE");
		eval("iwpriv", "ra0", "set", "IEEE8021X=0");
		eval("iwpriv", "ra0", "set", "SSID=ASUS_OTS");
		/* --- Cherry Cho modified in 2007/2/12. --- */
...
int OTSPacketHandler(int sockfd)
{
	...
	switch(phdr->OpCode)
	{
		case NET_CMD_ID_EZPROBE:
		{
		     ...
		     ezprobe_res->isNotDefault = atoi(nvram_safe_get("x_Setting")) | atoi(nvram_safe_get("wsc_config_state")); // for EZSetup to coexist w/ WSC
...
{ 
	...
	nvram_set("ots_running", "1");
	system("ots &");
	//s_eval2(ots_argv, NULL, 0, NULL);
	return 0;
}
