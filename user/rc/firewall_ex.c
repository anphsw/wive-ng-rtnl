...
// ~ oleg patch
	 ...
	 */
		...
		if (nvram_invmatch("misc_ping_x", "0"))	// qq
		{
...
*/
	...
	{   		
		...
		printf("\ncheck srcip = %s\n", srcip);	// tmp test
			...
			//if(strcmp(wan_if, "eth2.2") != 0)	// 0808 ham, it will cause pptp fail
         		//	fprintf(fp, "-A %s %s -i %s -o eth2.2 %s -j %s\n", chain, lanwan_timematch, lan_if, setting, ftype);
		}            
...
start_firewall_ex(char *wan_if, char *wan_ip, char *lan_if, char *lan_ip)
{
	...
	//2008.10 magic{
	#ifdef WEBSTRFILTER
       // if(web_filter())/* Cherry Cho added in 2007/12/28. */
        //        printf("Fail to add web filter!!\n");
	#endif
	//2008.10 magic}
	...
	{
		//if (nvram_get("misc_conntrack_x")==NULL) fputs("9012", fp); //2008.09 magic
		//else fputs(nvram_safe_get("misc_conntrack_x"), fp); //2008.09 magic
		fputs("15360", fp);
		fclose(fp);
	}
