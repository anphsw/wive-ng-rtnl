...
function markGroup(o, s, c, b)
{
	...
	if (b == " Add ")
	{
		...
		else if (s=='PPPoERouteList')
		{
			...
			else if (!validate_ipaddr(document.form.PPPConnection_x_PPPoEIP_0, "") || !validate_range(document.form.PPPConnection_x_PPPoEMask_0, 0, 32)) return false;
			else if (document.form.PPPConnection_x_PPPoEIP_0.value=="" || document.form.PPPConnection_x_PPPoEMask_0.value=="") bFlag=1;
		}
		else if (s=='GWStatic')
		{
			...
			if (window.top.isModel()!="WL520gc" && window.top.isModel()!="SnapAP")
			{
				...
				else if (document.form.vts_proto_x_0.value == "OTHER")
				{
					if (	!validate_ipaddr(document.form.vts_ipaddr_x_0, "") ||
						...
						!validate_range(document.form.vts_protono_x_0, 0, 255)
					) return false;
				...
				else
				{
					if (	!validate_ipaddr(document.form.vts_ipaddr_x_0, "") ||
						!validate_portrange(document.form.vts_port_x_0, "") ||
						!validate_range_sp(document.form.vts_lport_x_0, 1, 65535)
					) return false;
...
/*
			...
			else
			{
				...
				else if (document.form.filter_lw_srcip_x_0.value=="" && document.form.filter_lw_dstport_x_0.value=="") bFlag=1;
			}
*/
			if (document.form.filter_lw_num_x_0.value >= c) cFlag=1;
			else if (	!validate_iprange(document.form.filter_lw_srcip_x_0, "") ||
...
/*
		else if (s=='x_FUserList')
		{
			if (document.form.usb_ftpnum_x_0.value >= c) cFlag=1;
					...
					!validate_range(document.form.usb_ftpmaxuser_x_0, 1, 12)
			) return false;
			else if (	document.form.usb_ftpusername_x_0.value=="" ||
					document.form.usb_ftppasswd_x_0.value=="" ||
					document.form.usb_ftpmaxuser_x_0.value==""
			) bFlag=1;
...
*/
		else if (s=='x_QRuleList')
		{
			if (document.form.qos_rulenum_x_0.value >= c) cFlag=1;
					...
					!validate_range_sp(document.form.qos_minbw_x_0, 0, 1000000)
			) return false;
			else if (document.form.qos_maxbw_x_0.value=="" && document.form.qos_minbw_x_0.value=="") bFlag=1;
		...
		else if (s=='x_UQRuleList')
		{
			if (document.form.qos_urulenum_x_0.value >= c) cFlag=1;
					...
					!validate_range_sp(document.form.qos_uminbw_x_0, 0, 1000000)
			) return false;
			else if (document.form.qos_umaxbw_x_0.value=="" && document.form.qos_uminbw_x_0.value=="") bFlag=1;
		...
		else if (s=='x_USRRuleList')
		{
        		if (document.form.qos_rulenum_x_0.value >= c) cFlag=1;
                     		...
                     		document.form.qos_ip_x_0.value==""
                     	) bFlag=1;
...
function requireWANIP_0314(v)
{
	if(document.form.wan_route_x.value=="IP_Bridged")
		return 0;

	if (	v=='wan_ipaddr' || v=='wan_netmask' || v=='lan_ipaddr' || v=='lan_netmask' ||
...
function requireWANIP(v)	// oleg patch
{
        if(document.form.wan_route_x.value=="IP_Bridged")
                return 0;
	...
	if (	v=='wan_ipaddr' || v=='wan_netmask' || v=='lan_ipaddr' ||
		v=='lan_netmask' || v=='lan1_ipaddr' || v=='lan1_netmask')
    	{
			...
			return 1;
		if (document.form.wan_proto.value=="pppoe" && intoa(document.form.wan_ipaddr.value))
			return 1;
		if ((document.form.wan_proto.value=="pptp" || document.form.wan_proto.value == "l2tp") && document.form.wan_ipaddr.value!='0.0.0.0')
			return 1;
		else
			return 0;
    	}
...
function validate_ipaddr(o, v)
{
	...
	if (	v=='dhcp_start' || v=='dhcp_end' || v=='wan_ipaddr' ||
		v=='dhcp1_start' || v=='dhcp1_end' || v=='lan_ipaddr' || v=='lan1_ipaddr' || v=='staticip'
	)
	{
		//if(v1==255||v2==255||v3==255||v4==255||v1==0||v4==0||v1==127||v1==224)
		if((v!='wan_ipaddr')&&(v1==255||v2==255||v3==255||v4==255||v1==0||v4==0||v1==127||v1==224))	// oleg patch
		{
			alert(' <#JS_validip#>');
			o.value = "";
			o.focus();
		...
		if (requireWANIP(v) && ((v=='wan_ipaddr' &&  matchSubnet2(o.value, document.form.wan_netmask, document.form.lan_ipaddr.value, document.form.lan_netmask)) ||
	...
	if (requireWANIP(v) && ((v=='wan_netmask' &&  matchSubnet2(document.form.wan_ipaddr.value, o, document.form.lan_ipaddr.value, document.form.lan_netmask)) ||
...
function validate_ipaddr_final(o, v)
{
	...
	if (pos!=3 || num<0 || num>255)
	{
		...
		v4=num;
	if (	v=='dhcp_start' || v=='dhcp_end' || v=='wan_ipaddr' || v=='dhcp1_start' || v=='dhcp1_end' || 
		v=='lan_ipaddr' || v=='lan1_ipaddr' || v=='staticip' || v=='wl_radius_ipaddr' ||
...
//		wl_auth_mode_reconf_guest();
	...
	else if (document.form.current_page.value == "Advanced_WMode_Content.asp")
	{
		...
		{
			change_wireless_bridge(document.form.wl_mode_x.value, rcheck(document.form.wl_wdsapply_x), rcheck(document.form.wl_lazywds), 0);
		}
	}
...
//		}
	...
	else if (document.form.current_page.value == "Advanced_WWPS_Content.asp")
	{
		document.form.WPS_resetTimer.disabled=1;
		if (document.form.WL_auth_mode.value=="wpa" || document.form.WL_auth_mode.value=="radius" || document.form.WL_auth_mode.value=="shared")
		{
		...
		else if (document.form.wps_enable[0].checked == true)
		{
...
*/
	...
	else if (document.form.current_page.value == "Advanced_DHCP_Content.asp" || document.form.current_page.value == "Advanced_RDHCP_Content.asp")
	{
		final_flag = 1; 
	}
...
*/
	else if (document.form.current_page.value == "Advanced_DMZIP11g_Content.asp" || document.form.current_page.value == "Advanced_DMZIP_Content.asp")
	{
	...
	else if (document.form.current_page.value == "Main_AStatus_Content.asp")
	{
	...
	else if (document.form.current_page.value == "Main_Index_Content.asp")	// Jiahao patch
	{
		if ( (top.loadQuickSetup == "1") )
		{
			top.loadQuickSetup = "0";
			document.form.action.click();
		}
	}
	else if (document.form.current_page.value == "Advanced_WSecurity_Content.asp")	// Jiahao patch
	{
...
function change_wireless_bridge(m, a, r, mflag)
{
	if (a=="0" && r == "0" && mflag != 1)
	{
		...
		inputRCtrl2(document.form.wl_lazywds, 1); // set as 0
		inputRCtrl1(document.form.wl_wdsapply_x, 1);
		inputRCtrl1(document.form.wl_lazywds, 0); // disable
	}
	else
	{
		if (a=="0" && r == "0")
	...
	else if (document.form.current_page.value == "Advanced_LANWAN_Content.asp")
	{
		...
		if(!validate_ipaddr_final(document.form.lan_ipaddr, 'lan_ipaddr') ||
		   !validate_ipaddr_final(document.form.lan_netmask, 'lan_netmask')
			...
			return false;
		checkSubnet();
                inputCtrl(document.form.wan_ipaddr, 1); 	// oleg patch
                inputCtrl(document.form.wan_netmask, 1);	// oleg patch
                inputCtrl(document.form.wan_gateway, 1);	// oleg patch
	}
	else if (document.form.current_page.value == "Advanced_RLANWAN_Content.asp")
	{
		checkSubnet();
...
//				inputRCtrl1(document.form.misc_lpr_x, 1);
	...
	else if (document.form.current_page.value == "Advanced_WirelessGuest_Content.asp")
	{
		if (	!validate_ipaddr_final(document.form.lan1_ipaddr, 'lan1_ipaddr') ||
			...
			!validate_ipaddr_final(document.form.dhcp1_end, 'dhcp1_end')
		) return false;
...
function onSubmitApply(s)
{
	...
	if (document.form.current_page.value == "Advanced_PortMapping_Content.asp")
	{
		...
		if (action.value == "Set")
		{
			if (!validate_ipaddr(local, "") || !validate_portrange(port, ""))
			{
				return false;
			}
...
function change_common(o, s, v)
{
	...
	else if(s=="WLANConfig11b" && v == "wl_gmode")		
	{
		...
		if(o.value=='3')	// always enable WMM in N only mode
...
/*
		...
		{
			change_wireless_bridge2(document.form.wl_mode_x.value, rcheck(document.form.wl_wdsapply_x), 1, 1);
		}
		else
*/
	...
	{
		if (document.form.wl_wme[0].selected)
		{
...
{
	if (document.form.wl_auth_mode.value!="psk" && document.form.wl_auth_mode.value!="radius")	// by Jiahao. For RT-N15.
	{
  		change = 1;
  		window.top.pageChanged = 1;
	}
	...
	if (wflag=="1" && document.form.wl_auth_mode.value!="radius")	// by Jiahao. For RT-N15.
	{
		document.form.wl_phrase_x.focus();
	}
...
function change_wlweptype_sp(o, s)		// key_type patch by Jiahao
{
	...
	if (wflag=="1" && document.form.wl_auth_mode.value!="radius")
	{
		document.form.wl_phrase_x.focus();
	}
...
{
	widzard = window.open("Advanced_PortForwarding_Widzard.asp", "PortForwarding_Widzard","toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,copyhistory=no,width=600,height=400");
	if (!widzard.opener)
		widzard.opener = self;
}
...
{
	widzard = window.open("Advanced_VirtualServer_Widzard.asp", "VirtualServer_Widzard","toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,copyhistory=no,width=600,height=400");
	if (!widzard.opener)
		widzard.opener = self;
}
...
function openHelp(o, name)
{
	...
	widzard = window.open(urlstr, "Help_Widzard","toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,copyhistory=no,width=600,height=500");
	if (!widzard.opener)
		widzard.opener = self;
}
...
function wan_netmask_check(o) {
	...
	if (document.form.wan_ipaddr.value != '0.0.0.0' && (ip&lnm)==(lip&lnm))	// oleg patch
	{
		alert(' <#JS_validip#>');
...
*/
	...
	else if (s == "Advanced_LFirewall_Content.asp")
	{
		...
		ocument.form.FirewallConfig_WanLocalActiveTime.value = setTimeRange(
...
function startMenu(s)
{
	...
	{
		widzard = window.open("Advanced_PrinterSetup_Widzard.asp", "Printer Setup Widzard","toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,copyhistory=no,width=600,height=400");
		if (!widzard.opener)
			widzard.opener = self;
	}
}
...
function openWidzard(u, t)
{
	url = '"' + u + '"';
	title = '"' + t + '"';
	widzard = window.open(u, "Widzard","toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,copyhistory=no,width=600,height=400");
...
function openLink(s)
{
	...
	else if (s=='x_WImageSize')
	{
		...
		link = window.open(tourl, "WebCamera","toolbar=yes,location=yes,directories=no,status=yes,menubar=yes,scrollbars=yes,resizable=yes,copyhistory=no,width=640,height=560");
	}
	...
	else if (s=='x_WRemote')
	{
		...
		link = window.open(tourl, "RemoteMonitor","toolbar=no,location=no,directories=no,status=no,menubar=yes,scrollbars=yes,resizable=yes,copyhistory=no,top=0,left=0,width=" + screen.width + ",height=" + screen.height);
	}
	else if (s=='x_FIsAnonymous' || s=='x_FIsSuperuser')
	{
		...
		if (port!=21) tourl = tourl + ":" + port;
			link = window.open(tourl, "FTPServer","toolbar=yes,location=yes,directories=no,status=yes,menubar=yes,scrollbars=yes,resizable=yes,copyhistory=no,width=640,height=560");
	}
	if (!link.opener)
		link.opener = self;
}
...
{
	if (document.form.wl_auth_mode.value=="radius" || document.form.wl_auth_mode.value=="wpa")
...
function wl_rate_change()
{
	...
	if (document.form.wl_gmode.value == "1" || document.form.wl_gmode.value == "2")
	{
...
function changeDHCPClient()	// oleg patch
{    
...
function change_wan_type(v)
{
	if (v == "static")
	{
		...
		inputCtrl(document.form.wan_pppoe_options_x, 0);	// oleg patch
		inputCtrl(document.form.wan_pptp_options_x, 0);		// oleg patch
	...
	else if(v == "pppoe")
	{
		...
		inputCtrl(document.form.wan_pppoe_options_x, 1);	// oleg patch
		inputCtrl(document.form.wan_pptp_options_x, 0);		// oleg patch
		if(window.top.isModel3()!='WL550gE')
			inputRCtrl1(document.form.wan_pppoe_relay_x, 1);
		inputRCtrl1(document.form.x_DHCPClient, 1);		// oleg patch
		inputRCtrl2(document.form.x_DHCPClient, 1);		// oleg patch
	}
	else if(v == "pptp")
	{
		inputCtrl(document.form.wan_ipaddr, 1);
		...
		//	inputRCtrl1(document.form.wan_pppoe_relay_x, 1);
		inputCtrl(document.form.wan_pppoe_options_x, 1);		// oleg patch
		inputCtrl(document.form.wan_pptp_options_x, 1);			// oleg patch
		if(window.top.isModel3()!='WL550gE')				// oleg patch
			inputRCtrl1(document.form.wan_pppoe_relay_x, 1);	// oleg patch
		inputRCtrl1(document.form.x_DHCPClient, 1);			// oleg patch
		inputRCtrl2(document.form.x_DHCPClient, 1);			// oleg patch
	}
	else if(v == "l2tp")	// oleg patch
	{
		inputCtrl(document.form.wan_ipaddr, 1);
		inputCtrl(document.form.wan_netmask, 1);
		inputCtrl(document.form.wan_gateway, 1);
	...
	else if (v == "bigpond" || v == "cdma")
	{
		...
		if(window.top.isModel3()!='WL550gE')
			inputRCtrl1(document.form.wan_pppoe_relay_x, 1);
		inputRCtrl1(document.form.x_DHCPClient, 0);		// oleg patch
		inputRCtrl2(document.form.x_DHCPClient, 0);		// oleg patch
	}
	else
	{
		inputCtrl(document.form.wan_ipaddr, 0);
	...
	// oleg patch ~
	if ((v == "l2tp" || v == "pptp" || v == "pppoe") && document.form.wan_ipaddr.value == "")
	{
		inputRCtrl2(document.form.x_DHCPClient, 0);
	}
	if ((v == "l2tp" || v == "pptp" || v == "pppoe") && document.form.wan_ipaddr.value == "0.0.0.0")
	{
		inputRCtrl2(document.form.x_DHCPClient, 0);
        }
	changeDHCPClient();
...
{
	if (document.form.wl_guest_auth_mode_1.value=="radius" || document.form.wl_guest_auth_mode_1.value=="wpa")
