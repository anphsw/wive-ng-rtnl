...
void uuidstr_create(char *str, int len)
{
    ...
    u_int d[16];
    char  tmpstr[36];/* Cherry Cho added in 2007/4/9. */
    ...
    *((int *)&d[12]) = uuid_count++;
    ...
    #if 0/* Cherry Cho marked for using WSC source in 2007/4/10. */
    nvram_set("wsc_uuid",str);
    #else/* Cherry Cho added in 2007/4/9. WFADevice is another device.*/
    memset(tmpstr, 0x0, 36);
    sprintf(tmpstr,"0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", 
		(u_int8)d[0], (u_int8)d[1], (u_int8)d[2], (u_int8)d[3], (u_int8)d[4], 
		(u_int8)d[5], (u_int8)d[6], (u_int8)d[7], (u_int8)d[8], (u_int8)d[9], 
		(u_int8)d[10], (u_int8)d[11], (u_int8)d[12]+1, (u_int8)d[13], (u_int8)d[14], 
 		(u_int8)d[15]);
    nvram_set("wsc_uuid",tmpstr);
    #endif
}
