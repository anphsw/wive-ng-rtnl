...
/*
	if (	(value == NULL) || (strcmp(value, "") == 0) || (strcasecmp(value, "DB") == 0) ||
		(strcasecmp(value, "TW") == 0) || (strcasecmp(value, "US") == 0) ||
		(strcasecmp(value, "JP") == 0) || (strcasecmp(value, "KR") == 0) ||
		(strcasecmp(value, "DE") == 0) || (strcasecmp(value, "FR") == 0) || (strcasecmp(value, "GB") == 0) ||
		(strcasecmp(value, "CN") == 0) || (strcasecmp(value, "HK") == 0) || (strcasecmp(value, "AU") == 0))
	{
	...
	if ((value == NULL) || (strcmp(value, "") == 0) || (strcasecmp(value, "DB") == 0) || (strcasecmp(value, "JP") == 0))
	{
...
*/
	...
	if (	(strcasecmp(value, "CA") == 0) && (strcasecmp(value, "CO") == 0) && (strcasecmp(value, "DO") == 0) &&
		(strcasecmp(value, "GT") == 0) && (strcasecmp(value, "MX") == 0) && (strcasecmp(value, "NO") == 0) &&
		(strcasecmp(value, "PA") == 0) && (strcasecmp(value, "PR") == 0) && (strcasecmp(value, "TW") == 0) &&
		(strcasecmp(value, "US") == 0) && (strcasecmp(value, "UZ") == 0))
	{
		for (idx = 12; idx < 14; idx++)
		{
			sprintf(chstr, "%d", idx);
...
//		printf("get var: %s, name: %s, value: %s.\n", GetServiceId(sid), name, websGetVar(wp, name, NULL));	// N12 test
...
static int ej_notify_services(int eid, webs_t wp, int argc, char_t **argv){
	...
	if(restart_needed_bits != 0){
		no_run_str = 1;
csprintf("*** run ej_notify_services!\n");
// 2007.11 James {
		if((restart_needed_bits & RESTART_REBOOT) != 0){
/*csprintf("*** kill wanduck!\n");
...
// 2008.01 James }
	...
	if(no_run_str == 0)
csprintf("*** Don't run ej_notify_services!\n");
...
// 2008.01 James {
		...
		if(ppp_dir == NULL){
			//csprintf("wanlink_hook ppp_dir == NULL\n");
			status = 0;
			strcpy(statusstr, "Disconnected");
		}
...
// 2007.10 James }
		...
		if((s = socket(AF_INET, SOCK_DGRAM, 0)) < 0){
			//csprintf("wanlink_hook Open socket to kernel : open fail\n");
			status = 0;
			strcpy(statusstr, "Disconnected");
		}
		else{
...
// 2008.07 James. {
					...
					//csprintf("wanlink_hook manually_disconnect_wan=1\n");
					status = 0;
					strcpy(statusstr, "Disconnected");
				}
// 2008.07 James. }
...
static int ej_get_changed_status(int eid, webs_t wp, int argc, char_t **argv){
	...
	//char *disk_info = read_whole_file(PARTITION_FILE); 2008.08 magic
	//char *mount_info = read_whole_file("/proc/mounts"); 2008.08 magic
	u32 arp_info_len;// disk_info_len, mount_info_len; 2008.08 magic
	u32 arp_change;// disk_change; 2008.08 magic
...
static int ej_available_disk_names_and_sizes(int eid, webs_t wp, int argc, char_t **argv){
	...
	printf("webhook: available disk name s and sizes\n");	// tmp test
        disks_info = read_disk_data();
        if(disks_info == NULL){
		printf("disk info is NULL\n");	// tmp test
