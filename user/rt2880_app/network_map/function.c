	...
	setsockopt(sock_http, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout));//set receive timeout
        ...
        while(1)
        {
        	bzero(buffer, 512);
                ...
                if (getlen > 0)
                {
		#ifdef NETWORKMAP_DBG1
                        printf("Check http response: %s\n", buffer);
                #endif
...
int lpd515(unsigned char *dest_ip)
{
    	...
    	{
		#ifdef NETWORKMAP_DBG
        	printf("LPD515: socket create error.\n");
		#endif
        	return -1;
    	}
	...
	setsockopt(sockfd1, SOL_SOCKET, SO_SNDTIMEO, &timeout, sizeof(timeout));
    	...
    	{
		#ifdef NETWORKMAP_DBG
        	printf("LPD515: socket connect failed!\n");
		#endif
		return -1;
     	}
        ...
        {
		#ifdef NETWORKMAP_DBG
             	printf("LPD515: Send packet failed!\n");
		#endif
	    	return -1;
        }
        gettimeofday(&tv1, NULL);
...
int raw9100(unsigned char *dest_ip)
{
    ...
    {
	#ifdef NETWORKMAP_DBG
        printf("RAW9100: socket create error.\n");
	#endif
        return -1;
    }
    ...
    {
	#ifdef NETWORKMAP_DBG
        printf("RAW9100: socket connect failed!\n");
	#endif
    	close(sockfd2);
    	return -1;
    }
...
int FindAllApp(unsigned char *src_ip, unsigned char *dest_ip, P_IP_TABLE p_ip_tab)
{
	...
	MY_DEVICE_INFO my_dvinfo;
        ...
        if( !lpd515(dest_ip) ) {
	#ifdef NETWORKMAP_DBG
                printf("LPR Printer Server found!\n");
	#endif
                p_ip_tab->printer[p_ip_tab->num] = 1;
        }
        else if( !raw9100(dest_ip) ) {
	#ifdef NETWORKMAP_DBG
                printf("RAW Printer Server found!\n");
	#endif
                p_ip_tab->printer[p_ip_tab->num] = 2;
        }
        else {
	#ifdef NETWORKMAP_DBG
                printf("Printer Server not found!\n");
	#endif
	...
	#ifdef NETWORKMAP_DBG
		printf("Found iTune Server!\n");
	#endif
                p_ip_tab->itune[p_ip_tab->num] = 1;
        }
        else {
	#ifdef NETWORKMAP_DBG
                printf("No iTune Server!\n");
	#endif
