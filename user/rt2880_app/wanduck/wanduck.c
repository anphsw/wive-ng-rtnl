...
int chk_proto(){
	...
	if(!strcmp(tmp, "pptp")
		...
		//printf("[wdk] find proc name: %s\n", name);	// tmp test
                if(!strncmp(name, "pppd", 7) ||              // for PPPoE, PPTP
                        !strncmp(name, "bpalogin", 8)){ // for HeartBeat

			wan_status = nvram_safe_get("wan_status_t");
			if(strcmp(wan_status, "Connected") == 0)
			{
                        	return CONNED;
			}
			else
...
void get_related_nvram(){
	...
	strcpy(lan_netmask_t, nvram_safe_get("lan_netmask"));
