...
static int ntfs_fuse_getattr(const char *org_path, struct stat *stbuf)
{
	...
	if (stream_name_len < 0)
		return stream_name_len;
	memset(stbuf, 0, sizeof(struct stat64));
	ni = ntfs_pathname_to_inode(ctx->vol, NULL, path);
	if (!ni) {
		res = -errno;
		goto exit;
		...
		const unsigned dt_type __attribute__((unused)))
{
	...
	if (MREF(mref) == FILE_root || MREF(mref) >= FILE_first_user ||
			ctx->show_sys_files) {
		struct stat64 st = { .st_ino = MREF(mref) };
...
static void create_dev_fuse(void)
{
...
#ifdef __UCLIBC__
	{
		...
		if (stat64("/dev/misc", &st) && (errno == ENOENT)) {
			mode_t mask = umask(0); 
			mkdir("/dev/misc", 0775);
			umask(mask);
		}
...
static fuse_fstype get_fuse_fstype(void)
{
	...
	FILE *f = fopen64("/proc/filesystems", "r");
	if (!f) {
