...
extern int get_all_folder_in_mount_path(const char *const mount_path, int *sh_num, char ***folder_list){
	...
	//printf("[libdisk] get all folder in mount path [%s] \n", mount_path);	// tmp test
...
extern int get_permission(const char *const account,
	...
	//printf("<acc:%s> <mpath:%s> <foler:%s> <pro:%s>\n", account, mount_path, folder, protocol);	// tmp test
	//printf("\n---------------------\n");	// tmp test
	// 1. get the var file
	len = strlen(mount_path)+strlen("/.___var.txt")+strlen(account);
	var_file = (char *)malloc(sizeof(char)*(len+1));
	if(var_file == NULL){
		...
		printf("Can't allocate \"var_file\".\n");
		return -1;
	}
	sprintf(var_file, "%s/.__%s_var.txt", mount_path, account);
	var_file[len] = 0;
	...
	if(var_info == NULL){
		//printf("\"%s\" isn't existed or there's no content.\n", var_file);	// tmp test
		free(var_file);
		printf("\"%s\" isn't existed or there's no content.\n", var_file);
		return 0;
	}
	...
	//printf("var_info = [%s]\ntarget = [%s]\n", var_info, target);	// tmp test

	follow_info = upper_strstr(var_info, target);
	free(target);
	if(follow_info == NULL){
		//csprintf("No right about \"%s\" with \"%s\".\n", folder, account);
		//printf("No right about \"%s\" with \"%s\".\n", folder, account);	// tmp test
		free(var_info);
		return -1;
	}
	...
	if(follow_info[3] != '\n'){
		...
		//printf("The var info is incorrect.\nPlease reset the var file of \"%s\".\n", account);	// tmp test
		free(var_info);
		return -1;
	}
...
*/
		...
		printf("The protocol, \"%s\", is incorrect.\n", protocol);
		free(var_info);
		return -1;
	}
	free(var_info);
...
extern int set_permission(const char *const account,
	...
	if(var_info == NULL){
		...
		var_info = read_whole_file(var_file);
		if(var_info == NULL){
			csprintf("\"%s\" isn't existed or there's no content.\n", var_file);
			free(var_file);
			return -1;
		}
	}
	...
	if(result != 0){
		csprintf("Fail to \"run_ftp (set permission)\"!\n");
		return -1;
	}
...
extern int del_account(const char *const account){
	...
	result = eval("/sbin/run_ftp");
	if(result != 0){
		csprintf("Fail to \"run_ftp\"!(del account)\n");
		return -1;
	}
...
extern int mod_account(const char *const account, const char *const new_account, const char *const new_password){
	...
	result = eval("/sbin/run_ftp");
	if(result != 0){
		csprintf("Fail to \"run_ftp (mod account)\"!\n");
		return -1;
	}
...
extern int add_folder(const char *const mount_path, const char *const folder){
	...
	for(i = 0; i < acc_num; ++i){
		...
		if(strstr(var_info, target) != NULL){
			free(var_file);
			free(var_info);
...
extern int mod_folder(const char *const mount_path, const char *const folder, const char *const new_folder){
	...
	//char *upper_folder, *new_upper_folder;
	FILE *fp;
	char *follow_info, backup;
...
extern int how_many_layer(const char *basedir, char **mount_path, char **share){
	...
	if(layer >= SHARE_LAYER){
		...
		//printf("len = %d, follow_info=[%s]\n", len, follow_info);	// tmp test
