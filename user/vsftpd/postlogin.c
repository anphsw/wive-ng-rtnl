...
{
  //printf("[vsftpd] process post login\n");	// tmp test
...
// 2009 06 James{
        char *only_shown_right_commands[] = {
                        "LIST", "NLST", "STAT",
                        "PWD", "XPWD",
                        "CWD", "XCWD",
                        "CDUP", "XCUP",
        ...
        char *only_write_right_commands[] = {
                        "RETR",
                        0};
// 2009 06 James}
	...
	//printf("\n\n## <new cmd>\n");	// tmp test
	chk_path = 0;
        for(target_cmd = only_shown_right_commands; *target_cmd; ++target_cmd)
	{
		//printf(" (*chk*) (%d)(%d)\n", str_equal_text(&p_sess->ftp_cmd_str, *target_cmd), (!str_isempty(&p_sess->ftp_arg_str)));	// tmp test
	...
	if(strcmp(p_sess->ftp_cmd_str.PRIVATE_HANDS_OFF_p_buf, "RNTO") == 0)
	{
		chk_path = 1;
	}

	//printf("[ftp_arg_str_01_cmd] = [%s]\n", p_sess->ftp_cmd_str.PRIVATE_HANDS_OFF_p_buf);	// tmp test
	//printf("[ftp_arg_str_01_arg] = [%s]\n", p_sess->ftp_arg_str.PRIVATE_HANDS_OFF_p_buf);	// tmp test
	//printf("need chk path: %d\n", chk_path);	// tmp test
// 2008.05 James. for hiding the true path. {
	arg_buf = (char *)str_getbuf(&p_sess->ftp_arg_str);
	//if(strlen(arg_buf) > 0 && arg_buf[0] == '/'){
	if((chk_path == 1) && (strlen(arg_buf) > 0) && (arg_buf[0] != '-')){
		//printf("edit path\n");	// tmp test
		if(arg_buf[0] == '/')
		{
			...
			len = str_getlen(&p_sess->ftp_arg_str)+strlen(fullpath) + 1;
			arg_buf = (char *)malloc(sizeof(char)*(len+1));
			sprintf(arg_buf, "%s/%s", fullpath, str_getbuf(&p_sess->ftp_arg_str));
		}
		arg_buf[len] = 0;
...
// 2008.05 James. }
	//printf("\n[ftp_arg_str_02_cmd] = [%s]\n", p_sess->ftp_cmd_str.PRIVATE_HANDS_OFF_p_buf);	// tmp test
	//printf("[ftp_arg_str_02_arg] = [%s]\n", p_sess->ftp_arg_str.PRIVATE_HANDS_OFF_p_buf);	// tmp test
	...
	if(arg_buf != NULL && strlen(arg_buf) > 1 && arg_buf[len-1] == '/'){
		//printf(" chk argbuf with /\n");	// tmp test
		strncpy(fullpath, arg_buf, len-1);
		//printf("p 0: fullpath 01 = [%s]\n", fullpath);	// tmp test
...
/*	
	if(*arg_buf != 0){
		if(*arg_buf != '/')
		{
			...
			printf("p 1: fullpath 01 = [%s]\n", fullpath);	// tmp test
		}
		else
		{
			//sprintf(fullpath, "%s%s", POOL_MOUNT_ROOT, arg_buf);
			strcpy(fullpath, arg_buf);
			printf("p 2: fullpath 01 = [%s]\n", fullpath);	// tmp test
		}
	}
	else
	{
		getcwd(fullpath, PATH_MAX);
		//printf("p 3: fullpath 01 = [%s]\n", fullpath);	// tmp test
	}
*/
	getcwd(fullpath, PATH_MAX);

	//printf("now, full path is [%s]\n", fullpath);	// tmp test
        if((strcmp(p_sess->ftp_cmd_str.PRIVATE_HANDS_OFF_p_buf, "RNFR") == 0) && (!p_sess->is_anonymous))
        {
		sprintf(p_sess->full_path, "%s/%s", fullpath, p_sess->ftp_arg_str.PRIVATE_HANDS_OFF_p_buf);
	}
	else
		p_sess->full_path = fullpath;
	//printf("now 2, sess full path is [%s]\n", p_sess->full_path);	// tmp test
	...
	//printf("*get mount_path=[%s], share_name=[%s], layer=%d\n", mount_path, share_name, layer);	// tmp test
...
// 2009.06 James{
        for(target_cmd = only_write_right_commands; *target_cmd; ++target_cmd){
                if(str_equal_text(&p_sess->ftp_cmd_str, *target_cmd)){
                        if(layer >= SHARE_LAYER && !p_sess->is_anonymous){
                                user_right = get_permission(str_getbuf(&p_sess->user_str), mount_path, share_name, "ftp");
				//printf("[chk usr_right_02]= %d\n ", user_right);	// tmp test
                                //user_right = 3;
...
// 2009.06 James}
	...
	for(target_cmd = need_write_right_commands; *target_cmd; ++target_cmd){
		if(str_equal_text(&p_sess->ftp_cmd_str, *target_cmd)){
			//printf("p 7 chk [%s]\n", *target_cmd);	// tmp test
			//printf("chk layer (%d)(%d) (%d)\n", layer, SHARE_LAYER, (layer > SHARE_LAYER));	// tmp test
			//printf("chk anony (%d)\n", (!p_sess->is_anonymous));	// tmp test
			//printf("chk end\n");	// tmp test
			if((layer >= SHARE_LAYER) && (!p_sess->is_anonymous)){
				user_right = get_permission(str_getbuf(&p_sess->user_str), mount_path, share_name, "ftp");
				//printf("[chk usr_right_03]= %d\n ", user_right);	// tmp test
                                //user_right = 3;
				if(user_right < 2)
					cmd_ok = 0;
			}
...
VSFTPD_CMD:
	//printf("(cmdok:%d), [ftp_arg_str end_print] = [%s]\n", cmd_ok, p_sess->ftp_arg_str.PRIVATE_HANDS_OFF_p_buf);	// tmp test
...
lprintf("FTP_wrong_9.\n");
	...
	//printf("cur layer is %d(%d), anonymous is %d\n", p_sess->layer, SHARE_LAYER, p_sess->is_anonymous);	// tmp test
...
/*
	...
	printf("add folder [mnt_path=%s] [new_folder=%s] [backup=%s]\n", mount_path, new_folder, backup);	// tmp test
...
// 2007.08 James {
  ...
  int len;
	...
	//printf("[rnfr] full path is %s, arg str is %s\n", p_sess->full_path, p_sess->ftp_arg_str.PRIVATE_HANDS_OFF_p_buf);	// tmp test
  str_free(&tmp_str);
...
// 2007.08 James }
  if (retval == 0)
  {
	//printf("[rnfr, yes] rnfr_str is %s\n", tmp_str.PRIVATE_HANDS_OFF_p_buf);	// tmp test
    /* Yes */
...
// 2007.08 James {
  ...
  {
	//printf("RNTO chk 1\n");	// tmp test
	//printf("from:[%s]\n", p_sess->rnfr_filename_str.PRIVATE_HANDS_OFF_p_buf);	// tmp test
	//printf("to:[%s]\n", p_sess->ftp_arg_str.PRIVATE_HANDS_OFF_p_buf);	// tmp test
