 ...
 *
 * $Id: usbsock.c,v 1.4 2008/12/18 10:04:01 jiahao Exp $
 */
				...
				if ((USB_ENDPOINT_TYPE_MASK & dev->config[tmp_config].interface[tmp_int].altsetting[tmp_alt].endpoint[tmp_ep].bmAttributes) == USB_ENDPOINT_TYPE_BULK) {
					if ((USB_ENDPOINT_DIR_MASK & dev->config[tmp_config].interface[tmp_int].altsetting[tmp_alt].endpoint[tmp_ep].bEndpointAddress) == USB_ENDPOINT_OUT) {
						...
						if (Is_Canon && tmp_intclass == 7)
						{
							...
							ret = usb_bulk_write(udev, dev->config[tmp_config].interface[tmp_int].altsetting[tmp_alt].endpoint[tmp_ep].bEndpointAddress, (char*)pbuf, purb->UrbBulkOrInterruptTransfer.TransferBufferLength, 125);
						else
						...
						if (Is_HP && Is_HP_LaserJet_MFP && tmp_intclass == 7)
							ret = usb_bulk_read(udev, dev->config[tmp_config].interface[tmp_int].altsetting[tmp_alt].endpoint[tmp_ep].bEndpointAddress, (char*)pbuf, purb->UrbBulkOrInterruptTransfer.TransferBufferLength, 125);
						else
...
#if __BYTE_ORDER == __LITTLE_ENDIAN
	if ((fp = fopen("/tmp/hotplug_usb", "a")))
#else
	if ((fp = fopen("/var/hotplug_usb", "a")))
#endif
	{
		fprintf(fp, format);
		fclose(fp);
