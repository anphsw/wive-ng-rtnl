	...
	IN BOOLEAN isReassoc)
{
	...
	{
		pEntry->RrmEnCap.word = RrmEnCap.word;
	}
#endif // DOT11K_RRM_SUPPORT //
...
//                          1,                        &pAd->ApCfg.ErpIeContent,
	...
	if ((pFtCfg != NULL) && (pFtCfg->FtCapFlag.Dot11rFtEnable))
	{
		...
		/* Insert RIC. */
		if (FtInfo.RicInfo.Len)
		{
			ULONG TempLen;

			FT_RIC_ResourceRequestHandle(pAd, pEntry,
						(PUCHAR)FtInfo.RicInfo.pRicInfo,
						FtInfo.RicInfo.Len,
...
//		if (IS_FT_RSN_STA(pEntry) && (FtInfo.FtIeInfo.Len != 0))
		...
		{					
			...
			NdisMoveMemory(EvtReAssoc.OldApMacAddr, ApAddr, MAC_ADDR_LEN);
