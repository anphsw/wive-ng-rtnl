	...
	IN UCHAR RsnIeLen)
{
	...
	MgtMacHeaderInit(pAd, &AuthHdr, SUBTYPE_AUTH, 0, pHdr->Addr2, pHdr->Addr1);
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	for (apidx = 0; apidx < MAX_MBSSID_NUM; apidx++)
	{
		...
		sprintf(R0khIdBuf, "Ralink:%02x:%02x:%02x:%02x:%02x:%02x",
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	/* init spin lock */
	NdisAllocateSpinLock(&(pAd->ApCfg.FtTab.FT_R1khEntryTabLock));
	...
	IN INT32 PktLen)
{
	...
	REPORT_MGMT_FRAME_TO_MLME(pAd, Wcid, pOutBuffer, FrameLen, 0, 0, 0, 0);
	...
	PSTRING pBuffer)
{
	...
	for (Loop = 0; Loop < MAX_MBSSID_NUM; Loop++)
	{
		STRING tok_str[16];
		...
		*/
		NdisZeroMemory(tok_str, 16);
		sprintf(tok_str, "FtMdId%d", Loop + 1);
		...
		*/
		NdisZeroMemory(tok_str, 16);
		sprintf(tok_str, "FtR0khId%d", Loop + 1);
