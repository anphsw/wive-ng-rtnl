    ...
    PRTMP_ADAPTER pAd)
{
	...
	*/
	...
	{
 		// requested by Gary: if Rssi0 > -35, set RF_R27[1:0]=00, RF_R28[1:0]=00
	...
	IN PRTMP_TX_RATE_SWITCH	pTxRate)
{
	...
	DBGPRINT_RAW(RT_DEBUG_TRACE,("DRS: Aid=%d, APMlmeSetTxRate - CurrTxRateIdx=%d, MCS=%d, STBC=%d, ShortGI=%d, Mode=%d, BW=%d \n",
	...
	IN PRTMP_TX_RATE_SWITCH	pNextTxRate)
{
	if (pEntry->eTxBfEnCond != 0) {
		//DBGPRINT(RT_DEBUG_TRACE, ("-----> eTxBFProbing\n"));

		//DBGPRINT(RT_DEBUG_TRACE, ("eTxBFProbing eTxBfEnCond = %x, bfState = %x, noSndgCnt = %x, noSndgCntThrd = %x \n",
		//							pEntry->eTxBfEnCond, pEntry->bfState, pEntry->noSndgCnt, pEntry->noSndgCntThrd));

		//DBGPRINT(RT_DEBUG_TRACE, ("eTxBFProbing DebugTxBFCount = %d, DebugTxNormalCount = %d \n",
		//							pEntry->DebugTxBFCount, pEntry->DebugTxNormalCount));
	}

	pEntry->DebugTxBFCount = 0;
	pEntry->DebugTxNormalCount = 0;
    ...
    //
...
#endif // APCLI_SUPPORT //
#ifdef WDS_SUPPORT
			&& ((!IS_ENTRY_WDS(pEntry)) || !WDS_IF_UP_CHECK(pAd, pEntry->MatchWDSTabIdx))
#endif // WDS_SUPPORT //
			)
			return;
		...
		// Sync with Rory. In order to solve the issue that the throughput drop dramatically at middle-long rage.
                if (pTable == RateSwitchTable11N3S)
                {
                        ...
                        {
				APMlmeSelectRateSwitchTable11N3SReplacement(&pTable);
...
//UpRateIdx = DownRateIdx = 13;
		...
		else if ((Rssi > -65) && (pCurrTxRate->Mode >= MODE_HTMIX) && pEntry->perThrdAdj == 1)
		{
		...
		if (pAd->MacTab.Size == 1)
		{
			DBGPRINT_RAW(RT_DEBUG_TRACE,("DRS:Aid=%d, TxRetransmit=%ld, TxFailCount=%ld, TxSuccess=%ld, rssi0=%d, rssi1=%d, rssi2=%d \n",
 ...
 */		
		{
			DBGPRINT_RAW(RT_DEBUG_TRACE,("DRS: Before- CurrTxRateIdx=%d, MCS=%d, STBC=%d, ShortGI=%d, Mode=%d, TrainUp=%d, TrainDown=%d, NextUp=%d, NextDown=%d, CurrMCS=%d, TxBF=%c, mcsGroup=%d, TxQuality=%d,  PER=%ld%%\n",
    ...
    IN PRTMP_ADAPTER pAd)
{
	...
	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) //how to know which content is valid?by the conditions following" //only associated STA counts"
	{
		pEntry = &pAd->MacTab.Content[i]; //poiont to information of the individual station
		...
		//printk("Execute intelligent rate adaption!--APDynamicTxRateSwitching\n");
 ...
 */		
		{
			DBGPRINT_RAW(RT_DEBUG_TRACE,("DRS: Before- CurrTxRateIdx=%d, MCS=%d, STBC=%d, ShortGI=%d, Mode=%d, TrainUp=%d, TrainDown=%d, NextUp=%d, NextDown=%d, CurrMCS=%d, PER=%ld%%\n",
				CurrRateIdx,
				pCurrTxRate->CurrMCS,
				pCurrTxRate->STBC,
				pCurrTxRate->ShortGI,
        ...
        if (TxTotalCnt <= 15)
        {
			...
			if (TxRateIdx != pEntry->CurrTxRateIndex)
			{
				...
				APMlmeSetTxRate(pAd, pEntry, pNextTxRate);
		        	//DBGPRINT(RT_DEBUG_WARN, ("c-> %d\n", pNextTxRate->CurrMCS));				
				//DBGPRINT_RAW(RT_DEBUG_TRACE,("DRS: TxTotalCnt <= 15, switch to MCS%d according to RSSI (%d), RssiOffset=%d\n", pNextTxRate->CurrMCS, Rssi, RssiOffset));
			}
		...
		{
			pEntry->CurrTxRateStableTime = 0;  //redudant
			pEntry->TxRateUpPenalty = 0;           // no penalty
			pEntry->LastSecTxRateChangeAction = 2; // rate DOWN
			pEntry->TxQuality[pEntry->CurrTxRateIndex] = 0;
...
*/
		{
			DBGPRINT_RAW(RT_DEBUG_TRACE,("   QuickDRS: Before- CurrTxRateIdx=%d, MCS=%d, STBC=%d, ShortGI=%d, Mode=%d, TrainUp=%d, TrainDown=%d, NextUp=%d, NextDown=%d, CurrMCS=%d,  PER=%ld%%\n",
				CurrRateIdx,
				pCurrTxRate->CurrMCS,
				pCurrTxRate->STBC,
				pCurrTxRate->ShortGI,
