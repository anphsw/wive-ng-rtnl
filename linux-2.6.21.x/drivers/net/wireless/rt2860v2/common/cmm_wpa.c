			...
			else
			{
				...
				{			
	                ...
	                if (peerKeyInfo.KeyType == GROUPKEY)
	                {
						...
						{
							WpaDeriveGTK(pAd->ApCfg.MBSSID[apidx].GMK, (UCHAR*)pAd->ApCfg.MBSSID[apidx].GNonce, pAd->ApCfg.MBSSID[apidx].Bssid, pAd->ApCfg.MBSSID[apidx].GTK, TKIP_GTK_LENGTH);
    ...
    IN ULONG			TimeInterval) 
{
	...
	if ((pEntry->AuthMode == Ndis802_11AuthModeWPA2) && (pEntry->PMKID_CacheIdx != ENTRY_NOT_FOUND))
	{
    	...
    	INC_UINT16_TO_ARRARY(EAPOLPKT.KeyDesc.KeyDataLen, 6 + LEN_PMKID);    			
    	INC_UINT16_TO_ARRARY(EAPOLPKT.Body_Len, 6 + LEN_PMKID);
	}
#endif // CONFIG_AP_SUPPORT //
