/*
 * This is NAT offload engine for independed conntrack. Based nikicankik@gmail.com expressnat code.
 * Adapted to independed conntrack sfstudio@mail.ru for Wive-NG-RTNL firmware.
...
extern void *single_start(struct seq_file *p, loff_t *pos);
extern void *single_next(struct seq_file *p, void *v, loff_t *pos);
extern void single_stop(struct seq_file *p, void *v);
...
static int proc_nat_status_open (struct inode *inode, struct file *file)
    {
    int  ret=0;
    ret  = single_open(file, proc_nat_status_entry_show, NULL);
...
static inline int exp_fast_path_output(struct sk_buff *skb)
{
	...
	if (net_ratelimit())
		printk(KERN_DEBUG "expnat_finish_output: No header cache and no neighbour!\n");
	kfree_skb(skb);
	return -EINVAL;
}
#else 
...
int inline alg_is_registered(struct sk_buff *skb)
    ...
    int ret = 0;
   	...
   	if (skb->nfct)
        { 
        exp = (struct nf_conntrack_expect *)skb->nfct;
