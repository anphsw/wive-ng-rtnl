...
static struct nf_hook_ops client_limit_ops = 
{
...
#else
static struct workqueue_struct *clients_poll_wq;
			     ...
			     int (*okfn)(struct sk_buff *))
{
    ...
    if ((0 != strcmp(skb->dev->name, "br0"))
        || (0 == strcmp(skb->dst->dev->name, "br0")))
    {
    ...
    { 
		...
		if (0 == memcmp(skb->mac.raw + ETH_ALEN, dev->dev_mac, ETH_ALEN))
		{
	...
	list_for_each_safe(cur, next, list)
    {
        ...
        if ((0 == memcmp(skb->mac.raw + ETH_ALEN, client->client_mac, ETH_ALEN))
            && (client->client_ip == skb->nh.iph->saddr))
        {
            UNLOCK_CLIENT;
            return NF_ACCEPT;
        }
    ...
    if (client_num < limit)
    {
        ...
        memcpy(client->client_mac, skb->mac.raw + ETH_ALEN, ETH_ALEN);

        CLIENT_ADD(client, dev_type);
    }
    else
    {
		DEBUGP("\nFUNC: %s, LINE: %d, packet blocked for client limit, mac: %2x:%2x:%2x:%2x:%2x:%2x, ip: 0x%x, dev_type: %u\n",
            __FUNCTION__, __LINE__,
		    (skb->mac.raw + ETH_ALEN)[0],
		    (skb->mac.raw + ETH_ALEN)[1],
		    (skb->mac.raw + ETH_ALEN)[2],
		    (skb->mac.raw + ETH_ALEN)[3],
		    (skb->mac.raw + ETH_ALEN)[4],
		    (skb->mac.raw + ETH_ALEN)[5],
		    skb->nh.iph->saddr,
		    dev_type);
        UNLOCK_CLIENT;
        return NF_DROP;
...
static int __init init(void)
{
	...
	if (nf_register_hooks(&client_limit_ops, 1) < 0) 
    {
	...
	proc_dev->owner = THIS_MODULE;
    
	proc_config->write_proc = do_write_config;
	proc_config->owner = THIS_MODULE;
...
error:
	...
	nf_unregister_hooks(&client_limit_ops, 1);
