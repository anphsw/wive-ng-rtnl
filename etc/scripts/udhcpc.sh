...
case "$1" in
	...
	if [ "$FULL_RENEW" = "1" ] || [ ! -f /tmp/routes_applied ]; then
	    ...
	    # after route table modification reapply replace DGW (workaround for some ISP`s)
	    if [ "$REPLACE_DGW" = "1" ] && [ "$FULL_RENEW" = "1" ]; then
		if [ "$ROUTELIST_ST" != "" ] || [ "$ROUTELIST_CS" != "" ]; then
		    ROUTELIST="$ROUTELIST_FGW $ROUTELIST_DGW $ROUTELIST_ST $ROUTELIST_CS $ROUTELIST_DGW"
		    $LOG "Apply route list. And replace DGW (static routes recived)."
		else
		    ROUTELIST="$ROUTELIST_FGW $ROUTELIST_DGW"
